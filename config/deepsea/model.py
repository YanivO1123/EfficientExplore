...
class EfficientZeroNet(BaseNet):
    def __init__(
        ...
        # projection
        in_dim = num_channels * observation_shape[1] * observation_shape[
            2]  # num_channels * math.ceil(observation_shape[1] / 16) * math.ceil(observation_shape[2] / 16)
...
class FullyConnectedEfficientExploreNet(BaseNet):
    def __init__(self,
        ...
        self.reward_rnd_target_network = mlp(self.input_size_reward_rnd, fc_rnd_layers[:-1], fc_rnd_layers[-1],
                                             init_zero=False,
                                             momentum=momentum)
        self.value_rnd_network = mlp(self.input_size_value_rnd, fc_rnd_layers[:-1], fc_rnd_layers[-1], init_zero=False,
                                     momentum=momentum)
        self.value_rnd_target_network = mlp(self.input_size_value_rnd, fc_rnd_layers[:-1], fc_rnd_layers[-1],
                                            init_zero=False,
                                            momentum=momentum)
