...
namespace tree{
    ...
    CNode::CNode(){
        ...
        //MuExplore: Store the uncertainty variables.
    ...
    /*
     * TODO: Add additional function:

        CRoots::CRoots(int root_num, int action_num, int pool_size, bool mu_explore){
        this->root_num = root_num;
        this->action_num = action_num;
        this->pool_size = pool_size;
        this->mu_explore = mu_explore;
        ...
        for(int i = 0; i < root_num; ++i){
            ...
            this->roots.push_back(CNode(0, action_num, &this->node_pools[i], mu_explore));
        }
    }
    */
    ...
    void CRoots::prepare_explore(float root_exploration_fraction, const std::vector<std::vector<float>> &noises, const std::vector<float> &value_prefixs, const std::vector<std::vector<float>> &policies, const std::vector<float> &value_prefixs_uncertainty){
        for(int i = 0; i < this->root_num; ++i){
            // TODO: Setup policies to be equal 1/num actions
    ...
    void update_tree_q(CNode* root, tools::CMinMaxStats &min_max_stats, float discount){
        ...
        while(node_stack.size() > 0){
            ...
            if(node != root){
                ...
                //MuExplore: update min_max_stats with respect to the value + gamma^2 * value_uncertainty
                if (node->mu_explore){
...
//                min_max_stats.update(qsa);
    ...
    //MuExplore: a cbatch_back_propagate function that passes beta
    void cbatch_back_propagate(int hidden_state_index_x, float discount, const std::vector<float> &value_prefixs, const std::vector<float> &values, const std::vector<std::vector<float>> &policies, tools::CMinMaxStatsList *min_max_stats_lst, CSearchResults &results, std::vector<int> is_reset_lst, const std::vector<float> &value_prefixs_uncertainty, const std::vector<float> &values_uncertainty, float beta){
        for(int i = 0; i < results.num; ++i){
            //TODO: Modify policies to be a vector of 1/ actions, unless it's already passed from the python
